...
document.querySelector('#clearBtn').addEventListener('click',()=>{document.querySelectorAll('[data-save]').forEach(el=>{localStorage.removeItem('blocktrace_'+el.dataset.save); if(el.type==='checkbox')el.checked=false; else el.value='';});}); document.querySelector('#printBtn').addEventListener('click',()=>window.print());
