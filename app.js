...
function renderFindings(data){const rulesHtml=data.rules.length?data.rules.map(name=>`<div class="result-item"><b>${escapeRuleHtml(name)}</b><em>RULE</em></div>`).join(''):'<p>No YARA rule declarations found in the loaded text.</p>';const stringsHtml=`<div class="hit-list"><strong>STRINGS FOUND</strong><div>${data.strings.length?data.strings.map(value=>`<span class="hit">${escapeRuleHtml(value)}</span>`).join(''):'<span class="meta">No named strings found.</span>'}</div></div>`;const hitHtml=data.hits.length?`<div class="hit-list"><strong>ACTIVE TARGET MATCHES</strong><div>${data.hits.map(hit=>`<span class="hit">${escapeRuleHtml(hit.value)} × ${hit.count}</span>`).join('')}</div></div>`:'<div class="hit-list"><strong>ACTIVE TARGET MATCHES</strong><p>No configured indicators were found.</p></div>';byId('ruleResults').innerHTML=rulesHtml+stringsHtml+hitHtml;}
renderTargets();
