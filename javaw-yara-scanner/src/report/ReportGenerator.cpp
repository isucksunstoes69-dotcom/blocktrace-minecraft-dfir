...
std::string GenerateHtmlReport(const scanner::ScanSummary& summary, const std::string& outPath) {
    ...
    for (const auto& d : summary.detections) {
        ...
        if (uniqueDetections.find(key) == uniqueDetections.end()) {
            ...
            ud.details = d.details;
            ud.location = d.location;
            ud.totalHits = d.hits;
            ...
            if (uniqueDetections[key].location.empty() && !d.location.empty()) {
                uniqueDetections[key].location = d.location;
            }
    ...
    for (const auto& d : sortedDetections) {
        ...
        row << "<div class='log-row " << SeverityClass(d.severity) << " " << categoryClass << "'"
            ...
            << " data-details='" << EscapeHtml(d.details) << "'"
        ...
        if (!d.location.empty()) {
            row << "<span class='loc-hint'>" << EscapeHtml(d.location) << "</span>";
        }
        row << "</div>\n";
    ...
    std::string phaseTimingStr;
    for (const auto& pt : summary.phaseTimings) {
        if (!phaseTimingStr.empty()) phaseTimingStr += " &nbsp;|&nbsp; ";
        char buf[64];
        sprintf_s(buf, "%.1fs", pt.seconds);
        phaseTimingStr += EscapeHtml(pt.name) + " " + buf;
    }
    ...
    const std::string html =
        ...
        "--red:#e05555;--yellow:#c8b040;--blue:#5588d8;--green:#40c070;--orange:#e07a30;}"
        ...
        ".loc-hint{max-width:38%;flex-shrink:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;direction:rtl;text-align:left;color:var(--muted);font-family:Consolas,ui-monospace,Menlo,monospace;font-size:11px;opacity:.85;}"
        ...
        "<div class='detail-box' id='locationBox' style='display:none'><div class='detail-k' style='margin-bottom:5px'>File Location</div><div class='detail-v' id='d_loc'>-</div></div>"
        "<div class='detail-box' id='jvmSourceBox' style='display:none'><div class='detail-k' style='margin-bottom:5px'>Source Details</div><div class='detail-v' id='d_jvm_source'>-</div></div>"
        "</div>"
        "</main></div>"
        "<div class='footer'>" + phaseTimingStr + "</div></div>"
        ...
        "title.innerHTML=tab==='detects'?'Detects Logs':tab==='system'?'System Integrity':tab==='bypass'?'Bypass Logs':tab==='warnings'?'Warnings Logs <span style=\\'font-size:10px;opacity:0.6;font-weight:400\\'></span>':'Suspicious Logs';}"
        ...
        "var loc=row.dataset.loc||'';"
        "document.getElementById('d_loc').textContent=loc||'-';"
        "document.getElementById('locationBox').style.display=loc?'block':'none';"
        "var jvmSrc=row.dataset.details||'';"
        "document.getElementById('d_jvm_source').textContent=jvmSrc||'-';"
        "document.getElementById('jvmSourceBox').style.display=jvmSrc?'block':'none';"
