...
void JavaApp::Shutdown(){ if(m_scanThread.joinable()) m_scanThread.join(); ImGui_ImplOpenGL3_Shutdown(); ImGui_ImplGlfw_Shutdown(); ImGui::DestroyContext(); if(m_window)glfwDestroyWindow(m_window); glfwTerminate(); }
...
void JavaApp::ScanRuleFile(){
    ...
    if(m_scanning || m_scanThread.joinable()) return;
    m_hasResult=false; m_reportPath.clear(); m_status="Scanning rule file...";
    m_scanStarted=std::chrono::steady_clock::now(); m_scanning=true; m_scanReady=false;
    const std::string path=m_rulePath.data();
    m_scanThread=std::thread([this,path](){ m_result=scanner::ScanRuleTextFile(path); m_scanning=false; m_scanReady=true; });
}
void JavaApp::FinishScan(){
    if(!m_result.ok){ m_status=m_result.error; return; }
    bool found=false; for(const auto& item:m_result.indicators) found|=item.hits>0;
    if(found){ m_reportPath="BlockTraceRuleFindings.html"; report::GenerateYaraHtmlReport(m_result,m_reportPath); report::OpenInDefaultBrowser(m_reportPath); m_status="Indicators found. Findings website opened in your default browser."; }
    else m_status="No requested indicators found in the selected rule file.";
...
void JavaApp::DrawLoadingWindow(float){
    ImGui::SetNextWindowPos({0,0}); ImGui::SetNextWindowSize(ImGui::GetIO().DisplaySize);
    ImGui::Begin("Scanning",nullptr,ImGuiWindowFlags_NoDecoration|ImGuiWindowFlags_NoMove); DrawTitleBar();
    auto ws=ImGui::GetWindowSize(); auto* dl=ImGui::GetWindowDrawList(); ImVec2 sz{600,145}, tl{(ws.x-sz.x)/2,(ws.y-sz.y)/2};
    dl->AddRectFilled(tl,{tl.x+sz.x,tl.y+sz.y},IM_COL32(20,2,10,220),10); dl->AddRect(tl,{tl.x+sz.x,tl.y+sz.y},IM_COL32(185,65,105,160),10);
    const char* title="P1AE javaw"; auto ts=ImGui::CalcTextSize(title); ImGui::SetCursorPos({(ws.x-ts.x)/2,tl.y+16}); ImGui::TextColored({.92f,.86f,.95f,1},"%s",title);
    float elapsed=std::chrono::duration<float>(std::chrono::steady_clock::now()-m_scanStarted).count(); float progress=std::min(.92f,.12f+elapsed*.25f);
    ImGui::SetCursorPos({tl.x+25,tl.y+55}); ImGui::ProgressBar(progress,{sz.x-50,8},"");
    const char* dots=(static_cast<int>(elapsed*2)%3==0)?".":(static_cast<int>(elapsed*2)%3==1)?"..":"..."; char label[128]{}; sprintf_s(label,"Scanning rule file%s  %d%%  (%.1fs)",dots,static_cast<int>(progress*100),elapsed);
    ImGui::SetCursorPos({(ws.x-ImGui::CalcTextSize(label).x)/2,tl.y+85}); ImGui::TextColored({.75f,.70f,.82f,1},"%s",label); ImGui::End();
}
...
void JavaApp::Run(){float last=(float)glfwGetTime();while(!glfwWindowShouldClose(m_window)){glfwPollEvents();if(m_scanReady.exchange(false)){if(m_scanThread.joinable())m_scanThread.join();FinishScan();}float now=(float)glfwGetTime(),dt=now-last;last=now;int w,h;glfwGetFramebufferSize(m_window,&w,&h);m_starfield.Resize(w,h);ImGui_ImplOpenGL3_NewFrame();ImGui_ImplGlfw_NewFrame();ImGui::NewFrame();m_starfield.Render(ImGui::GetBackgroundDrawList(),{0,0},{(float)w,(float)h},now);if(m_scanning)DrawLoadingWindow(dt);else DrawMainWindow(dt);ImGui::Render();glViewport(0,0,w,h);glClear(GL_COLOR_BUFFER_BIT);ImGui_ImplOpenGL3_RenderDrawData(ImGui::GetDrawData());glfwSwapBuffers(m_window);}}
}
