...
void JavaApp::ScanRuleFile(){ if(!m_rulePath[0]){m_status="Provide a readable rule-file path first.";return;} m_result=scanner::ScanRuleTextFile(m_rulePath.data()); m_hasResult=m_result.ok; m_status=m_result.ok?"Rule text parsed locally. Review indicator coverage below.":m_result.error; }
